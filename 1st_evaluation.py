...
def prepare_prf(pred_dict, ref_dict):
    ...
    """
    preds = {k: v['entity_answers'] for k, v in list(pred_dict.items())}
    refs = {k: v['entity_answers'] for k, v in list(ref_dict.items())}
    return preds, refs
...
def get_metrics(pred_result, ref_result, task, source):
    ...
    if source == 'both':
        ...
        for question_id, info in list(ref_result.items()):
    ...
    if task == 'main' or task == 'all' or task == 'description':
        ...
        for k, pred, ref in zip(keys, preds, refs):
            ...
            k_metric = [(k + '|' + key, v) for key, v in list(m.items())]
...
def prepare_bleu(pred_result, ref_result, task):
    ...
    for qid, ans in list(ref_dict.items()):
...
def get_yesno_result(qid, pred_result, ref_result):
    ...
    def _uniq(li, is_ref):
        ...
        if is_ref:
            ...
            for k, v in left:
                dict_li[k] += v
            uniq_li = [(k, v) for k, v in list(dict_li.items())]
        return uniq_li
...
def format_metrics(metrics, task, err_msg, sources):
    """
...
#     """   
    ...
    task = 'main'
    try:
        pred_result = read_file(submit_file,task)
        ref_result = read_file(test_file,task,is_ref=True)
        #  sources = ['both','search','zhidao']
        sources = ['zhidao']
        if task not in set(['main','all']):
            sources = sources[:1]
        for source in sources:
            metrics[source] = get_metrics(
                pred_result, ref_result, task, source)
    ...
    final_result = format_metrics(metrics, task, err, sources)
